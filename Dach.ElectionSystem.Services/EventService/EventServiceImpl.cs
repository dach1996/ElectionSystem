            ...
            await Task.Run(() =>
             {
                 if (eventBase.DateRegister > eventBase.DateMaxRegisterCandidate)
                     throw new CustomException(MessageCodesApi.IncorrectData, ResponseType.Error, HttpStatusCode.BadRequest, $"La fecha de Registro de candidatos no puede ser menor a la fecha de registro del evento");
                 if (eventBase.DateRegister > eventBase.DateMaxRegisterParticipants)
                     throw new CustomException(MessageCodesApi.IncorrectData, ResponseType.Error, HttpStatusCode.BadRequest, $"La fecha de Registro de participantes no puede ser menor a la fecha de registro del evento");
                 ...
                 if (eventBase.DateRegister > eventBase.DateMaxVote)
                     throw new CustomException(MessageCodesApi.IncorrectData, ResponseType.Error, HttpStatusCode.BadRequest, $"La fecha Máxima para votar no puede ser menor a la fecha de registro del evento");
